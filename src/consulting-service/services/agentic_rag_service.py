...
Agentic RAG with web search fallback:
  ...
  Step 3 — Reflect  : LLM judges if context is sufficient
  Step 4 — Retry    : Reformulate and search ChromaDB again (max 2x)
  Step 5 — Web      : If still insufficient, search the web
...
class AgenticRAGService:
    ...
    async def ask(self, question: str, top_k: int | None = None) -> dict:
        ...
        while state.iterations < MAX_ITERATIONS:
            ...
            logger.info("Iteration %d — sufficient: %s | %s",
                        state.iterations, state.sufficient, state.reasoning[:80])
    ...
    async def _plan(self, question: str) -> list[str]:
        prompt = f"""You are a search query planner for a technical maintenance manual database.

Decompose the question into 1 to 3 short focused search queries that together cover all aspects.
Return a JSON array of strings only. No explanation. No markdown.
Example: ["vibration velocity limits", "measurement frequency range"]
...
JSON array:"""
    ...
    async def _reflect(self, state: AgentState) -> None:
        context_preview = "\n\n".join(
            f"[{c['source']}, page {c['page']}]: {c['text'][:200]}"
            for c in state.chunks[:6]
        )
...
Reply with JSON only: {{"sufficient": true or false, "reasoning": "one sentence why"}}
...
JSON:"""
        ...
        try:
            ...
            logger.warning("Reflection failed (%s) — assuming sufficient.", e)
            state.sufficient = True

    # ── Step 4: Reformulate ───────────────────────────────────────────────────

    async def _reformulate(self, state: AgentState) -> list[str]:
        prompt = f"""You are improving search queries for a maintenance manual database.

Original question: {state.question}
Previous queries: {state.queries}
Why context was insufficient: {state.reasoning}

Generate 1 to 2 new different search queries targeting the missing information.
...
JSON array:"""
    ...
    async def _retrieve_from_web(self, state: AgentState) -> None:
        """Search the web via Tavily and add results to state.web_results."""
        if not settings.tavily_api_key:
            logger.warning("TAVILY_API_KEY not set — skipping web search.")
            return

        try:
            from tavily import TavilyClient   # imported here — optional dependency
            client  = TavilyClient(api_key=settings.tavily_api_key)
            # Use the original question for web search — broader than sub-queries
...
- Cite manual excerpts like (Manual Excerpt 1, page 5) and web sources like (Web Source 2).

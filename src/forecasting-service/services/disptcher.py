    ...
    if rul < config.rul_critical:
        if is_high_conf:
            return DispatchDecision(
                ...
                message=f"CRITICAL ALERT: Machine {mid} - {ft.upper()} detected. RUL: {rul:.1f}h. Immediate action required.",
                should_alert=True, channels=["sms", "push", "dashboard"],
            )
        else:
            return DispatchDecision(
                machine_id=mid, alert_level=AlertLevel.SENSOR_NOISE,
                mean_rul=rul, confidence=conf, failure_type=ft,
                message=f"INVESTIGATE: Machine {mid} reports critical RUL ({rul:.1f}h) but confidence is LOW ({conf:.0%}). Check for sensor noise.",
                should_alert=True, channels=["dashboard"],
            )
        ...
        if is_high_conf:
            return DispatchDecision(
                ...
                mean_rul=rul, confidence=conf, failure_type=ft,
                message=f"MAINTENANCE REQUIRED: Machine {mid} - {ft}. RUL: {rul:.1f}h. Schedule within {config.rul_warning}h.",
            ...
            return DispatchDecision(
                ...
                should_alert=False, channels=["dashboard"],
            )
        ...
        return DispatchDecision(
            ...
            mean_rul=rul, confidence=conf, failure_type=ft,
            message=f"STATUS {'HEALTHY' if is_high_conf else 'MONITORING'}: Machine {mid} operating normally. RUL: {rul:.1f}h.",
            should_alert=False,
            channels=[] if is_high_conf else ["dashboard"],
        )
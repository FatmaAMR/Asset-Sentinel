...
export default function UploadCard() {
  const { ingest, isIngesting, ingestResult, ingestError, resetIngest } =
    useKnowledgeStore();
      ...
      <div className="relative z-10">
        <div className="w-16 h-16 bg-primary text-white rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-xl shadow-primary/30">
          <span className="material-symbols-rounded text-3xl">
            {isIngesting ? "hourglass_top" : "cloud_upload"}
          </span>
        </div>

        <h2 className="text-2xl font-display font-bold mb-2 text-white dark:text-light">
        ...
        <div className="flex justify-center gap-4">
          ...
          >
            ...
            <span className="material-symbols-rounded">
              {isIngesting ? "sync" : "arrow_forward"}
            </span>
          </button>
        </div>
      </div>

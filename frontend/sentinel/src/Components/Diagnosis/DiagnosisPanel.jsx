...
const QUICK_ACTIONS = [
  { label: "Compare with 2022 overhaul logs", question: "What does the maintenance manual say about comparing overhaul logs and bearing wear patterns?" },
  { label: "Show vibration spectrogram", question: "What vibration frequency patterns indicate bearing failure according to ISO 20816?" },
  { label: "Estimate MTBF", question: "How is mean time between failures calculated for rolling element bearings?" },
];
...
export default function DiagnosisPanel() {
  ...
  useEffect(() => {
    askConsulting(
      "What are the symptoms and root causes of bearing failure in industrial turbines? What are the recommended maintenance actions?",
      4
    );
  }, []);

  const handleSend = async (questionOverride) => {
    const text = questionOverride ?? q;
    if (!text.trim() || isLoading) return;
    setQ("");
    setChatHistory((prev) => [...prev, { role: "user", text }]);
    await askConsulting(text, 4);
  ...
  const displayRootCause = rootCause || "Awaiting Sentinel AI diagnosis...";
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400">
                {isLoading && !rawAnswer ? (
                  <span className="text-primary animate-pulse">Analyzing with RAG engine...</span>
                ) : (
          ...
          <div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-full flex items-center gap-1">
            ...
            {sources.length > 0 ? (
              sources.slice(0, 2).map((s, i) => (
                <button key={i} className="px-3 py-1 rounded-full text-xs font-semibold bg-white dark:bg-slate-700 shadow-sm text-primary truncate max-w-[120px]">
                  {s.file?.replace(".pdf", "").slice(0, 15)}…
                </button>
              ))
            ) : (
              <>
                <button className="px-3 py-1 rounded-full text-xs font-semibold bg-white dark:bg-slate-700 shadow-sm text-primary">SKF-M-2023</button>
                <button className="px-3 py-1 rounded-full text-xs font-semibold text-slate-500">ISO-10816</button>
              </>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6 mb-8">
          <div className="p-6 rounded-3xl bg-primary/5 border border-primary/10">
            ...
            {isLoading && !rawAnswer ? (
              ...
              <p className="text-lg font-semibold leading-relaxed">{displayRootCause}</p>
            )}
          </div>
        ...
        <div className="space-y-6">
          <div className="flex items-start gap-4">
            <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center flex-shrink-0">
              <span className="material-symbols-rounded text-sm text-slate-500">format_list_bulleted</span>
            </div>
            <div className="space-y-4 flex-1">
              ...
              {isLoading && !rawAnswer ? (
                <div className="space-y-3">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="h-4 bg-slate-100 dark:bg-slate-800 rounded animate-pulse" style={{ width: `${60 + i * 12}%` }} />
                  ))}
                </div>
              ) : (
            ...
            <div className="flex-1">
              {citation ? (
                <>
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed italic">"{citation.text}..."</p>
                </>
              ) : (
                <>
                  <h4 className="text-sm font-bold mb-1">Citing: SKF Rolling Bearing Manual Sec. 7.4</h4>
                  <p className="text-xs text-slate-500 leading-relaxed italic">
                    "Vibration signatures exhibiting high-frequency modulation at the Ball Pass Frequency Inner race (BPFI) typically indicate fatigue spalling..."
                  </p>
                </>
              )}
            </div>
          ...
          {error && (
            <div className="p-4 rounded-2xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 text-sm flex items-center gap-3">
              ...
              <span>{error}</span>
              <button
                onClick={() => askConsulting("What are the symptoms and root causes of bearing failure?", 4)}
          ...
          {chatHistory.length > 2 && (
            <div className="space-y-3 pt-2">
              <h4 className="font-bold text-sm text-slate-500 uppercase tracking-wider">Follow-up</h4>
